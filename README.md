...
**Features:**
- 🛏️ Auto-wake: Commands automatically wake sleeping vehicles and retry
- 🔄 Smart retry: Up to 3 retry attempts with increasing wait times
- 💡 Helpful tips: Suggests waking vehicle from Tesla app for deeply sleeping vehicles
...
If you're using **Windows Subsystem for Linux (WSL)**, use these commands to install the tools:
...
### Simple Flow (What Happens When You Send a Command)
...
│                                                                     │
│  ┌──────────────────────┐                                          │
│  │   tesla-cli.go       │  ← You run this + choose command         │
│  │ (Interactive Menu)   │                                          │
│  └──────────┬───────────┘                                          │
│             │ HTTPS Request + OAuth token                          │
│             ↓                                                       │
│  ┌──────────────────────────────────────────────┐                 │
│  │    Tesla Proxy (Docker/Kubernetes)           │                 │
│  │  - Receives command                          │                 │
│  │  - Signs with private key (Vehicle Command)  │                 │
│  │  - Encrypts payload                          │                 │
│  │  - Adds Tesla auth headers                   │                 │
│  └──────────┬───────────────────────────────────┘                 │
│             │ HTTPS + Signed Command                               │
│             ↓                                                       │
└─────────────────────────────────────────────────────────────────────┘
              │
              │ Over Internet
...
│  │  - Routes to vehicle via Tesla infrastructure│                 │
│  └──────────┬───────────────────────────────────┘                 │
│             │ OTA (Over-The-Air)                                   │
│             ↓                                                       │
│  ┌──────────────────────────────────────────────┐                 │
│  │    Your Tesla Vehicle (via LTE/WiFi)        │                 │
│  │  - Receives & decrypts command               │                 │
│  │  - Executes action (lock, unlock, etc)       │                 │
│  │  - Sends status back to Tesla servers        │                 │
│  └──────────────────────────────────────────────┘                 │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
              ↑
              │ Status returned to your CLI
              ├──────────────────────────────────┐
              │                                  │
            ✅ Success                         ❌ Error
```
...
┌──────────────┐
│ Run CLI      │
│ go run       │ (no tokens cached)
│ tesla-cli.go │
└──────┬───────┘
       │
       ↓
┌──────────────────────────────────────────────────────────────┐
│ CLI opens browser for Tesla login                           │
...
└──────┬───────────────────────────────────────────────────────┘
       │
       ↓
   [You copy authorization code]
       │
       ↓
┌──────────────────────────────────────────────────────────────┐
│ Paste code into CLI terminal                                │
│ CLI exchanges code for tokens via Tesla API                 │
└──────┬───────────────────────────────────────────────────────┘
       │
       ↓
┌──────────────────────────────────────────────────────────────┐
│ Tokens saved to tesla-tokens.json:                          │
│ {                                                            │
│   "access_token": "qts_xxxxx",                              │
│   "refresh_token": "cxl_xxxxx",                             │
...
│ (Tesla HTTP Server)      │     (or Kubernetes service)
│ - TLS encryption         │
│ - Command signing        │
│ - Protocol handling      │
└──────────┬───────────────┘
...
- **Docker Proxy / Kubernetes Service** - End-to-end encryption, cryptographic signing, protocol conversion
...
- **Tesla Fleet API** - Validates commands and routes to vehicles
- **Your Vehicle** - Executes commands via OTA (over-the-air) updates
...
Currently in menu:
- **Wake Vehicle** - `wake_vehicle` - Explicitly wake a sleeping vehicle
...
**Auto-wake Feature:** All commands automatically attempt to wake the vehicle if it's offline/asleep, then retry. No manual intervention needed!
...
### "vehicle unavailable: vehicle is offline or asleep" (HTTP 500)

**Problem**: Commands fail with vehicle unavailable error.

**Why**: Tesla vehicles enter sleep mode to preserve battery. API access isn't possible until the vehicle wakes.

**Solution (Automatic):**
The CLI automatically handles this! It will:
1. Wake the vehicle
2. Wait 8-14 seconds for it to respond
3. Retry the command up to 3 times

**If auto-wake is too slow:**
1. Wake the vehicle from your **Tesla app** first (faster)
2. Then run the command in the CLI

**Manual wake option:**
```bash
# Run the CLI and select option 1: Wake Vehicle
go run tesla-cli.go
# Choose: 1
```
...
- ✅ Docker containerized proxy (Docker Compose option)
- ✅ Kubernetes deployment (Kind + Helm)
- ✅ **Auto-wake on sleep** with automatic retry logic
- ✅ Proper VIN parsing (handles Windows line endings)
- ✅ Kubernetes Secrets management for TLS certificates
